...
#ifndef LIVEWALPAPER_CPP_
#define LIVEWALPAPER_CPP_
#include "Board.h"
#include "util.h"
#include <iostream>
#include <string>
...
void GameDisplay()/**/{
	...
	if (check == true) {
		...
		for (int i = 0; i < 8; i++) {
			...
			speed = rand() % 6 + 1;
			direction = rand() % 4;
			ptr[i]->setInitialLocation(aa/*aa*/, bb/*bb*/);
			ptr[i]->setColor(color);
			ptr[i]->setSize(radius/*radius*/, height/*height*/);
			ptr[i]->setSpeedAndInitialDirection(speed, direction);
		}
		...
		foreach object in objects
			foreach objectInner in (objects - object)
				check if coordiantes of object matches with ptrInner
			end
		end
		*/
	...
	for (int i = 0; i < 8; i++) {
		...
		ptr[i]->BoundaryCollision();

		for (int j = i + 1; j < 8; j++) {
			bool ptrDeleted = false;
			ptr[i]->checkAndProcessCollision(ptr[j], ptrDeleted);

			if (ptrDeleted) {
				aliveObjects[i] = false;
			}
		}
	}
...
int main(int argc, char*argv[]) {
	...
	glutCreateWindow("LIVE WALPAPER GAME"); // set the title of our game window
